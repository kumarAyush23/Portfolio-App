...
const NavBar = () => {
  ...
  <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed">
    <div>
        <h1 className="text-5xl text-amber-600 font-signature ml-2">Ayush</h1>
    </div>
    <ul className="hidden md:flex">
        {links.map(({ id,link }) => (
        <li key={id} className="px-4 cursor-pointer capitalize font-medium text-amber-600 hover:scale-105 duration-200">
          <Link to={link}
